...
class WeTextProcessingManager:
    ...
    def _run(self) -> None:
        try:
            ...
            logging.warning("WeTextProcessing not available, falling back to dummy normalization: %s", exc)
            self._available = False
            self._set_state(state="ready", message="WeTextProcessing fallback (not installed).", error=None)
